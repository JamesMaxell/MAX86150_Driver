...
# 这段代码的主要目的是使用红外线和红色数据来计算心率和血氧饱和度（SpO2）。它定义了一个名为calc_hr_and_spo2的函数，
# 该函数接受红外线和红色数据作为输入，并返回心率和血氧饱和度。
# 在这个函数中，首先计算红外线数据的直流平均值，并从数据中减去直流平均值以反转信号。然后，对反转后的信号进行4点移动平均，
# 以平滑数据。接下来，计算阈值并使用find_peaks函数在反转后的信号中找到峰值。
# 如果找到的峰值数量大于等于2，则计算峰值之间的平均间隔，并使用该间隔来计算心率。否则，将心率设置为-999，表示无法计算。
# 接下来，代码尝试计算血氧饱和度。它首先找到精确的最小值位置，然后寻找红外线-红色直流和红外线-红色交流信号以用于SpO2校准比率。
# 然后，在两个山谷位置之间找到最大值，并使用Ir和Red的AC分量与Ir和Red的DC分量之间的比率来计算SpO2。
# 最后，选择中位数作为比率值，并使用该比率值来计算血氧饱和度。如果比率值在2到184之间，则使用公式
# spo2 = -45.060 * (ratio_ave ** 2) / 10000.0 + 30.054 * ratio_ave / 100.0 + 94.845来计算血氧饱和度。
# 否则，将血氧饱和度设置为-999，表示无法计算
...
# find_peaks(x, size, min_height, min_dist, max_num)：这个函数用于在给定的数据中找到最多max_num个高度大于min_height且
# 相距至少min_dist的峰值。它首先调用find_peaks_above_min_height函数来找到所有高度大于min_height的峰值，
# 然后调用remove_close_peaks函数来删除相距小于min_dist的峰值
...
# find_peaks_above_min_height(x, size, min_height, max_num)：这个函数用于在给定的数据中找到所有高度大于min_height的峰值。
# 它通过遍历数据并检测潜在峰值的左边缘和右边缘来实现
...
# remove_close_peaks(n_peaks, ir_valley_locs, x, min_dist)：这个函数用于删除相距小于min_dist的峰值。
# 它首先按照峰值大小对峰值进行排序，然后遍历排序后的峰值并删除相距小于min_dist的峰值。
...
# 测试脚本，它使用预先定义的红外线和红色数据来调用calc_hr_and_spo2函数并计算心率和血氧饱和度。然后，它打印出有效的心率和血氧饱和度，
# 并在它们有效且不等于-999时打印出心率和血氧饱和度的值。
